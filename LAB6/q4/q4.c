...
//	len++;
	...
	int *idx_arr = (int *)malloc(sizeof(int)*n);

	for(i=0,k=0;i<len && k<n;i++)
		if(str[i] == ' ')
			idx_arr[k++] = i;
	idx_arr[k] = len;
	...
	cl_mem tbuff = clCreateBuffer(context,CL_MEM_WRITE_ONLY,len*sizeof(char),NULL,&ret);
	cl_mem idxbuff = clCreateBuffer(context,CL_MEM_READ_ONLY,n*sizeof(char),NULL,&ret);

	ret = clEnqueueWriteBuffer(cmdq,sbuff,CL_TRUE,0,len*sizeof(char),str,0,NULL,NULL);
	ret = clEnqueueWriteBuffer(cmdq,idxbuff,CL_TRUE,0,n*sizeof(char),idx_arr,0,NULL,NULL);
	...
	ret = clSetKernelArg(kernel,2,sizeof(cl_mem),(void *)&idxbuff);
	...
	ret = clEnqueueReadBuffer(cmdq,tbuff,CL_TRUE,0,len*sizeof(char),strres, 0, NULL,NULL);
